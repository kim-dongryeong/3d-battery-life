...
export function chargeStats(samples, level = 'rawcap') {
  ...
  const mk = () => Array.from({ length: 10 }, () => ({ rise: 0, sec: 0, adpSum: 0, adpSec: 0 }));
  ...
  for (let i = 1; i < samples.length; i++) {
    ...
    for (let k = 0; k < 10; k++) {                           // band k = (10k, 10k+10]
      ...
      for (const acc of [global, profiles[pk], classes[ck]]) {
        acc[k].rise += rise; acc[k].sec += sec;
        // 밴드별 어댑터 입력 평균(실측 PDTR) — "그때 어댑터가 포화였나"의 증거로 쓰인다:
        // 포화(입력≈정격)였다면 그 밴드의 관측 속도는 배터리 수용 한도가 아니라 어댑터 한도다
        if (a.adapterW != null && a.adapterW > 0.5) { acc[k].adpSum += a.adapterW * sec; acc[k].adpSec += sec; }
      }
    }
  }
  const fin = acc => {
    const byBand = {}, secByBand = {}, adpWByBand = {};
    let totalSec = 0;
    acc.forEach((bnd, k) => {
      totalSec += bnd.sec;
      secByBand[10 * k + 10] = Math.round(bnd.sec);
      if (bnd.sec > 0 && bnd.rise > 0) byBand[10 * k + 10] = +(bnd.rise / (bnd.sec / 60)).toFixed(4);   // %/min
      if (bnd.adpSec > 0) adpWByBand[10 * k + 10] = +(bnd.adpSum / bnd.adpSec).toFixed(2);              // 평균 입력 W
    });
    return { byBand, secByBand, adpWByBand, totalMin: Math.round(totalSec / 60) };
  };
