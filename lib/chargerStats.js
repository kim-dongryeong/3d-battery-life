...
// 세션 병합(buildSessionLinks) 판단용: chargerKey가 "구체 adapterId"(0이 아닌 truthy 정수)를 갖는지.
// 구체 id를 가진 키는 modelKeyOf가 family#adapterId로 이미 자기 계약끼리 정확히 묶으므로 세션 병합이
// 불필요하다. 오히려 세션 병합을 구체 id 쪽까지 허용하면, 연속 AC 구간에서 물리적으로 다른 두 충전기가
// 우연히 인접하거나(예: id0 'pd charger' 계약 다음 틱에 다른 충전기의 id28699 계약이 나타남) 재협상
// 정체 드롭아웃(구체 충전기가 잠깐 id0 'pd charger'로 찍힘)이 다리를 놓아 서로 다른 실물이 한 모델로
// 붕괴하는 사고가 난다(2026-08-04 kdr 실데이터 확인: Apple "35W USB-C Power Adapter"#28699와 Xtorm
// 보조배터리 id0 'pd charger'가 한 카드로 합쳐짐). 그래서 세션 병합은 "양쪽 다 구체 id 없음(id0/미상)"
// 일 때만 허용한다 — 정체 브리징(bridgeGenericIdentity)이 먼저 드롭아웃을 복원해 주므로, 이 게이트를
// 통과하고도 남는 잔여 케이스(예: id0 충전기의 서로 다른 메뉴 지문 계약들)만 세션 병합의 대상이 된다.
function hasConcreteId(key) {
  return !!parseKeyIds(key).adapterId;
}
...
// 교체 — 예: usb host 5V → PD 27W) 그 경계에서는 연결하지 않는다. 그리고 2026-08-04부터: 한쪽이라도
// 구체 adapterId를 가지면(hasConcreteId) 아예 세션 병합 대상에서 제외한다(위 설명 참고).
function buildSessionLinks(samples, runs) {
  ...
  for (const run of runs) {
    for (let i = 1; i < run.length; i++) {
      ...
      if (hasConcreteId(ka) || hasConcreteId(kb)) continue;   // 구체 id 있는 쪽은 세션 병합 안 함 — modelKeyOf가 이미 정확히 묶음
