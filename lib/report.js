...
function buildRuns(samples, dayOf) {
  ...
  for (const s of samples) {
    ...
    if (!cur) cur = { points: [] };
    cur.points.push({ t: s.t, pct: s.pct, watts: s.watts, tempC: s.tempC, loadPct: s.loadPct, ac: s.ac, charging: s.charging });
  }
...
function bucketStats(sessions) {
  ...
  for (const s of sessions) {
    ...
    for (let i = 1; i < pts.length; i++) {
      ...
      if (a.pct == null || b.pct == null || b.pct > a.pct) continue;   // discharge/flat only
      const dtMin = (b.t - a.t) / 60, dPct = a.pct - b.pct;
      if (!(dtMin > 0)) continue;                                       // skip duplicate/non-increasing timestamps
      if (dPct === 0) {
        // flat pair: pct is an INTEGER from ioreg, so most 60s samples repeat the same value.
        // That dwell time is real discharge time — dropping it (old behavior) collapsed every
        // band's rate to ~1%/sample-interval, i.e. "10% ≈ 10분" for ALL bands on real data.
        const k = Math.min(9, Math.max(0, Math.floor((100 - a.pct) / 10)));  // band (lo, hi] containing pct
        acc[k].min += dtMin;
        continue;
      }
      for (let k = 0; k < 10; k++) {
        const bandHi = 100 - 10 * k, bandLo = 90 - 10 * k;             // band (bandLo, bandHi]
        const top = Math.min(a.pct, bandHi), bot = Math.max(b.pct, bandLo);
