...
function buildRuns(samples, dayOf) {
  ...
  for (const s of samples) {
    ...
    cur.points.push({ t: s.t, pct: s.pct, watts: s.watts, tempC: s.tempC, loadPct: s.loadPct, ac: s.ac, charging: s.charging, lowPower: s.lowPower,
      ...
      rawCap: s.rawCap ?? null, rawMax: s.rawMax ?? null,   // 게이지 IC mAh — 구간 전력량의 '게이지 검산'(쿨롱 델타)용
      voltage: s.voltage, amperage: s.amperage, powerW: s.powerW, systemW: s.systemW ?? null, adapterW: s.adapterW ?? null,   // 배터리 V/A/W(항상) + 시스템/어댑터 W(앱 실행 중 SMC)
      // raw per-METHOD inputs (SMC, app-running only) so the tooltip/graph can show & switch methods:
      // powerW=수지(PDTR−PSTR) · ioreg V×I(셀 실측) · PPBR(방전 전용) · SMC 실측 어댑터 V/A · ioreg 공칭 어댑터
      ioregW: s.ioregW ?? null, ioregA: s.ioregA ?? null, ppbrW: s.ppbrW ?? null,
      dcInV: s.dcInV ?? null, dcInA: s.dcInA ?? null, adapterWnom: s.adapterWnom ?? null, adapterVnom: s.adapterVnom ?? null,
      adapterName: s.adapterName ?? null, familyCode: s.familyCode ?? null,
      adapterId: s.adapterId ?? null });   // lib/adapters.js chargerKey()와 클라이언트에서 같은 지문을 재현하려면 필요(웹 툴팁 별명 매칭)
  }
...
function bucketStats(sessions, level = 'rawcap') {
  const lvl = p => (level === 'pct' ? p.pct : (p.cap != null ? p.cap : p.pct));   // 정밀 mAh%(기본) / 정수%(옵션)
...
export function buildReport(samples, opt = {}) {
  const level = opt.level === 'pct' ? 'pct' : 'rawcap';   // 전역 정밀도 (정밀 mAh 기본 / 정수% 옵션)
  ...
  return {
    ...
    runs,
    health,
    buckets: bucketStats(sessions, level),
    bucketsEarly: bucketStats(early, level),
    bucketsRecent: bucketStats(recent, level),
  };
}
