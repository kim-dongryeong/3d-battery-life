...
export function buildSpans(samples, opt = {}) {
  ...
  for (const s of samples) {
    ...
    if (!cur) cur = [];
    cur.push({ t: s.t, lvl, w: Math.abs(s.watts ?? 0), tc: s.tempC ?? null, raw: s });
  }
  close();
  return spans;
}
...
export function atomicIntervals(spans) {
  ...
  for (const span of spans) {
    ...
    for (let i = 1; i < span.length; i++) {
      ...
      const avgW = (a.w + b.w) / 2, avgTc = (a.tc != null && b.tc != null) ? (a.tc + b.tc) / 2 : (a.tc ?? b.tc);
      const cuts = [];
      ...
      for (const nx of [...cuts, { t: b.t, lvl: b.lvl }]) {
        ...
        if (time > 0) {
          ...
          atoms.push({ spanId, day: dayKey(pt.t), band: bandHigh(mid), subbin: Math.min(100, Math.max(1, Math.ceil(mid))), drop, time, ws: avgW * time, ts: avgTc != null ? avgTc * time : 0, hasT: avgTc != null });
        }
        pt = nx;
      }
    }
...
export function analyzeRates(samples, opt = {}) {
  ...
  for (const [k, as] of aByK) {
    ...
    const sumWs = as.reduce((x, a) => x + a.ws, 0), sumTs = as.reduce((x, a) => x + (a.hasT ? a.ts : 0), 0), sumTtime = as.reduce((x, a) => x + (a.hasT ? a.time : 0), 0);
    const bySub = new Map();
    ...
    perCell.push({
      ...
      whPerPct: sumDrop > 0 ? round(sumWs / 3600 / sumDrop, 4) : null,   // Wh consumed per 1% drained (load-normalized aging proxy)
      avgW: sumTime > 0 ? round(sumWs / sumTime, 2) : null,
  ...
  return { opt: { level: opt.level ?? 'pct', period, gLin, gate }, spans: spans.length, atoms: atoms.length, perCell, periods, byBand };
}
