...
export function applyLiveSMC(s, useAvg = false) {
  try {
    ...
    const batW = pick('batteryWAvg', 'batteryW');               // SMC PPBR: battery rail magnitude
    if (smc.tempC != null) s.tempC = smc.tempC;
    if (sysW != null) s.systemW = sysW;
    if (adpW != null) s.adapterW = adpW;
    s.smc = true;
    if (batW != null && sysW != null) {                         // signed battery power
      const charging = (adpW ?? 0) - sysW >= 0;
      const signed = charging ? batW : -batW;
