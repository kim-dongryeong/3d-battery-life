...
// tray app, which publishes live-smc.json every ~1s with BOTH the instantaneous reading AND a rolling
...
export function sample(now = Date.now()) {
  ...
  if (rec.ac) {
    const adBlk = (io.match(/"AdapterDetails"\s+=\s+\{([^}]*)\}/) || [])[1] || '';
    const aw = adBlk.match(/"Watts"=(-?\d+)/), av = adBlk.match(/"AdapterVoltage"=(-?\d+)/);
    if (aw) rec.adapterWnom = +aw[1];
    if (av) rec.adapterVnom = +(+av[1] / 1000).toFixed(3);
    // adapter TYPE (종류): the human-readable Name/Description ("30W USB-C Power Adapter"). Recorded per
    // minute while plugged in so the report knows WHICH adapter over time. (Serial is skipped — privacy + waste.)
    const an = adBlk.match(/"Name"="([^"]*)"/) || adBlk.match(/"Description"="([^"]*)"/);
    if (an) rec.adapterName = an[1];
  }
  return rec;
}
