...
// Override with the JOULE_DATA env var (legacy BATTERY_DATA still read as a fallback).
// (Shipped demo .jsonl are assets, not here.)
import os from 'node:os';
import path from 'node:path';
import fs from 'node:fs';

export function userDataDir() {
  return process.env.JOULE_DATA || process.env.BATTERY_DATA || path.join(os.homedir(), 'Library', 'Application Support', 'joule');
}
