// battery-life-smcd — launchd(KeepAlive) 상주 데몬. 트레이 앱이 꺼져 있어도 기록 파이프라인을
// 완전하게 유지한다:
...
//   2) 60초마다 옆의 `battery-life sample`(one-shot 레코더)을 직접 실행해 분당 기록을 보장 —
...
fn data_dir() -> std::path::PathBuf {
    ...
    std::path::Path::new(&home).join("Library/Application Support/3d-battery-life")
}

fn tray_app_running() -> bool {
    std::process::Command::new("/usr/bin/pgrep")
        .args(["-x", "battery-life-desktop"])
...
// 분당 레코더: 이 바이너리 옆의 battery-life(컴파일 CLI)를 one-shot으로 실행.
